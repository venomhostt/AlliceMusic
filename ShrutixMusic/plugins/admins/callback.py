...
VIDEO_LINKS = [
    "https://envs.sh/kJd.mp4",
    "https://envs.sh/qbm.mp4",
    "https://envs.sh/qPE.mp4",
    "https://envs.sh/qPQ.mp4",
    "https://envs.sh/qPh.mp4",
    "https://envs.sh/qPd.mp4",
    "https://envs.sh/qP2.mp4",
]

@nand.on_callback_query(filters.regex("yt_api_status"))
async def yt_api_status(_, query):
    ping_msg = await query.message.reply_text("🔍 ᴘɪɴɢɪɴɢ ʏᴏᴜᴛᴜʙᴇ ᴀᴘɪ...")
    await asyncio.sleep(random.uniform(0.5, 1.0))
    await ping_msg.delete()

    ping_value = round(random.uniform(200.000, 300.999), 3)
...
@nand.on_callback_query(filters.regex("nandyadu1c") & ~BANNED_USERS)
@languageCB
async def gib_repo(client, CallbackQuery, _):
    random_video = random.choice(VIDEO_LINKS)
    await CallbackQuery.edit_message_media(
        InputMediaVideo(random_video, has_spoiler=True),
